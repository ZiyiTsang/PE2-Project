...
namespace Power_Store.Models
{
    public class Repo
    {
        ...
        // filter base  on key word
        public Dictionary<int, Item> filter(string key_word)
        {
           
            return Product_list.Where(x => x.Value.ItemName.ToLower().Contains(key_word.ToLower())).ToDictionary(x => x.Key, x => x.Value);
        }
